...
import { Menu, ChevronDown, Zap, Shield, Cpu, Activity, Settings, ArrowRight } from "lucide-react";
...
const Header = () => {
  ...
  const isSolutionsRoute = tecnologias.some((tec) => location.pathname === tec.path);
  ...
  return (
    <motion.header 
      ...
      className={`fixed z-50 transition-all duration-500 ease-in-out flex justify-center w-full ${
        scrolled 
          ? "top-0 sm:top-4 px-0 sm:px-4" 
          : "top-0 px-0"
      }`}
    >
      <div 
          ...
          <Link to="/" className="flex items-center gap-2 z-50 group" onClick={() => setIsOpen(false)}>
            ...
            >
              <img 
                src={logo} 
                alt="Logo Avolt" 
                className={`w-auto object-contain transition-all duration-500 drop-shadow-sm group-hover:drop-shadow-md ${scrolled ? "h-10 sm:h-11" : "h-12 sm:h-14"}`} 
              />
            </motion.div>
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-1 lg:gap-2">
            {["Home", "Quem Somos", "Momentos de Sucesso"].map((item, i) => {
              ...
              return (
                ...
                >
                  <span className={`relative z-10 text-[15px] lg:text-base font-semibold transition-colors duration-300 ${isActive ? "text-blue-700" : "text-slate-600 group-hover:text-blue-700"}`}>
                    {item}
                  </span>
            ...
            <DropdownMenu>
              ...
              >
                ...
                {tecnologias.map((tec, index) => {
                  const Icon = tec.icon;
                  return (
                    <DropdownMenuItem key={index} asChild className="rounded-xl mb-1 focus:bg-blue-50/80 focus:text-blue-700 transition-all duration-200 cursor-pointer p-0">
                      <Link to={tec.path} className="flex items-center gap-3 w-full px-3 py-3 group/item">
                        ...
                        <span className="font-semibold text-[15px]">{tec.name}</span>
                      </Link>
                    </DropdownMenuItem>
                  )
                })}
          ...
          <div className="md:hidden flex items-center">
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
              <SheetTrigger asChild>
                <Button variant="ghost" size="icon" className="relative group rounded-full h-12 w-12 hover:bg-slate-100/80 transition-all">
                  <Menu className="h-6 w-6 text-slate-700 group-hover:text-blue-600 transition-colors" />
                </Button>
              </SheetTrigger>
              <SheetContent side="right" className="w-[85vw] sm:w-[400px] bg-white/95 backdrop-blur-3xl border-l-white/60 shadow-2xl p-0">
                    ...
                    {["Home", "Quem Somos", "Momentos de Sucesso"].map((item, i) => {
                      ...
                      return (
                        <Link 
                          ...
                          className={`text-[19px] font-bold py-3.5 px-4 rounded-xl transition-all ${isActive ? "bg-blue-50 text-blue-700 shadow-sm border border-blue-100/50" : "text-slate-800 hover:bg-slate-50 hover:text-blue-600"}`}
                        >
                          {item}
                        </Link>
                      );
